...
class PlanckSH0ESJointLikelihood:
    def __init__(self, planck_data, H0_shoes=73.04, sigma_shoes=1.04):
        if isinstance(planck_data, dict):
            ...
            self.planck = {
                ...
                "cov": df[[f"cov{i}{j}" for i in range(3) for j in range(3)]]
                      .values.reshape(3, 3),
            }
    ...
    def log_likelihood_planck(self, model):
        try:
            ...
            logp = -0.5 * chi2
            print(f"  Planck → R={R_model:.4f}, lA={lA_model:.4f}, ombh2={ombh2_model:.6f} | logp={logp:.4f}")
            return logp
    ...
    def __call__(self, theta):
        ...
        try:
            logp_planck = self.log_likelihood_planck(None)  # placeholder - will be fixed later
